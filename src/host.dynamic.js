...
return {
  ...
  apply(ctx) {
     ...
     * 每个引擎能配哪些连接字段。
     *
     * **dsh 不在这张表里**：它的模型和连接由 dsh 自己那套设置（侧边栏「设置」）管，
     * 插件去写只会和它打架 —— 而且 dsh 跑在 dsh 自己的进程里，插件没有注入点。
     *
     * 空串一律表示「不覆盖，用这个引擎自己的配置」：
     *   - claude → `~/.claude/settings.json` 的 env
     *   - codex  → `~/.codex/config.toml` 的 provider
     * 所以默认全是空串，也就是「什么都不改」。
     */
    const AGENT_CONNECTION_FIELDS = {
      claude: ['baseUrl', 'apiKey', 'authToken'],
      codex: ['baseUrl', 'apiKey', 'provider'],
    }

    function emptyConnection(id) {
      const connection = {}
      for (const field of (AGENT_CONNECTION_FIELDS[id] || [])) connection[field] = ''
      return connection
    }

    /**
     * 宽松读：agents.json 是外部输入，一份写坏的连接配置最坏只该让那个引擎退回
     * 「用自己配置」，不该让整个注册表读不出来。所以逐字段挑，坏值直接丢。
     */
    function readConnection(id, raw) {
      const connection = emptyConnection(id)
      if (AGENT_CONNECTION_FIELDS[id] === undefined) return undefined
      if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return connection
      for (const field of AGENT_CONNECTION_FIELDS[id]) {
        const value = raw[field]
        if (typeof value === 'string' && value.trim().length > 0) connection[field] = value.trim()
      }
      return connection
    }

    /**
     * 严格写：设置面板提交上来的值。空串合法（＝不覆盖），其余只拒绝两件真会
     * 坏事的事 —— 类型不对、长到不像话。
     */
    function validateConnection(id, raw) {
      if (AGENT_CONNECTION_FIELDS[id] === undefined) {
        throw new Error(ENGINE_IDS.indexOf(id) === -1
          ? '未知引擎：' + id
          : id + ' 没有可配的连接字段（它的连接由它自己那套设置管）')
      }
      const connection = emptyConnection(id)
      if (raw === undefined || raw === null) return connection
      if (typeof raw !== 'object' || Array.isArray(raw)) throw new Error('连接配置必须是一个对象')
      for (const field of AGENT_CONNECTION_FIELDS[id]) {
        const value = raw[field]
        if (value === undefined || value === null || value === '') continue
        if (typeof value !== 'string') throw new Error(field + ' 必须是字符串')
        const trimmed = value.trim()
        if (trimmed.length > 500) throw new Error(field + ' 太长了（最多 500 字）')
        connection[field] = trimmed
      }
      return connection
    }
    ...
    function defaultAgents() {
      ...
      for (const id of ENGINE_IDS) {
        agents[id] = { name: DEFAULT_AGENT_NAMES[id], source: 'local' }
        const connection = readConnection(id, undefined)
        if (connection !== undefined) agents[id].connection = connection
      }
      return agents
    }
    ...
    async function readAgentsFile() {
      ...
      for (const id of ENGINE_IDS) {
        ...
        const connection = readConnection(id, entry.connection)
        if (connection !== undefined) agents[id].connection = connection
      }
      agentsCache = agents
      return agents
    ...
    function applyAgentsEdit(agents, edit) {
      ...
      const next = {}
      for (const key of ENGINE_IDS) {
        next[key] = { name: agents[key].name, source: agents[key].source }
        const connection = readConnection(key, agents[key].connection)
        if (connection !== undefined) next[key].connection = connection
      }
      const saved = { name: name, source: source }
      // connection **可选**：老的调用方（只传 id/name/source）不能被弄坏 ——
      // 不传就保持原样。
      if (edit.connection !== undefined) saved.connection = validateConnection(id, edit.connection)
      else if (next[id].connection !== undefined) saved.connection = next[id].connection
      next[id] = saved
      return next
    }
    ...
    async function agentsState() {
      ...
      return {
        ...
        agents: ENGINE_IDS.map((id) => ({
          id: id,
          name: agents[id].name,
          source: agents[id].source,
          // 没有连接字段的引擎给 null，客户端据此决定「这一栏不渲染连接输入框」。
          connection: agents[id].connection === undefined ? null : agents[id].connection,
        })),
      }
    }
    ...
    async function agentsEdit(edit) {
      const agents = await readAgentsFile()
      await writeAgentsFile(applyAgentsEdit(agents, edit))
      // 改完立刻刷新缓存，否则下一轮 spawn 读到的还是旧连接（它只认缓存）。
     ...
     * agents 的缓存。
     *
     * `launchSnapshot` 是同步的（它决定「换个配置要不要重建 Claude 进程」），
     * 没法在里面 await 读文件，所以这里存一份进程内的镜像。改完必须重读，
     * 否则「下一轮生效」会变成「重启才生效」。
     *
     * **惰性初始化**，不在声明处调 defaultAgents()：那是模块体执行期，而
     * `defaultAgents` 依赖的 `ENGINE_IDS` 声明在这个位置**之后**（见文件后段），
     * 在这里调用就是 TDZ 报错，整个插件起不来。启动时的预热在 apply 末尾。
     */
    let agentsCache = null

    /** 某个引擎的连接配置（同步，拿缓存）。缓存还没热就是「什么都没覆盖」。 */
    function connectionOf(id) {
      if (agentsCache === null) agentsCache = defaultAgents()
      const entry = agentsCache[id]
      return entry === undefined || entry.connection === undefined ? {} : entry.connection
    }

    /**
     * 写一份含密钥的 JSON，并把它设成 600。
     *
     * 这类文件里可能有 apiKey / authToken（用户选了明文存），所以不能靠 umask ——
     * 默认 umask 给的是 644，同机任何用户都读得到。先写临时文件、chmod、再 mv：
     * 中途不会出现权限更宽的窗口。
     */
    function writeSecretJsonFile(shellPath, value) {
      const text = JSON.stringify(value, null, 2) + '\n'
      const tmp = shellPath + '.tmp'
      return new Promise((resolve, reject) => {
        let proc
        try {
          proc = silenceStdin(subprocess.spawn({
            argv: ['/bin/sh', '-c', 'mkdir -p "$(dirname ' + shellPath + ')" && (umask 077 && cat > ' + tmp
              + ') && chmod 600 ' + tmp + ' && mv -f ' + tmp + ' ' + shellPath],
            cwd: '/',
            stdio: { stdin: 'pipe', stdout: 'ignore', stderr: 'ignore' },
            graceMs: 3000,
          }))
    ...
    function writeAgentsFile(value) {
      return writeSecretJsonFile(AGENTS_PATH, value)
    }

    /**
     * 把 claude 的连接覆盖写成一份 `--settings` 文件，返回路径；没有要覆盖的返回 null。
     *
     * 为什么落盘而不是 `--settings '{…}'` 内联：claude 进程是 broker 以
     * `node broker.mjs <dir> <argv…>` 拉起来的，内联 JSON 会让 apiKey 出现在
     * **进程命令行**里（`ps` 可见）。文件只传路径，key 不进 argv。
     *
     * 只放要覆盖的那几项 —— `--settings` 是**叠加**语义（实测：只覆盖
     * ANTHROPIC_BASE_URL 时，模型名仍来自用户的 settings.json），所以用户其它
     * 配置不会被冲掉。这点是必须的：进程环境覆盖不了 settings.json 里已有的键
     * （同样实测过），只有 `--settings` 压得住。
     */
    async function writeClaudeSettingsFile(connection) {
      const env = {}
      if (typeof connection.baseUrl === 'string' && connection.baseUrl.length > 0) env.ANTHROPIC_BASE_URL = connection.baseUrl
      if (typeof connection.apiKey === 'string' && connection.apiKey.length > 0) env.ANTHROPIC_API_KEY = connection.apiKey
      if (typeof connection.authToken === 'string' && connection.authToken.length > 0) env.ANTHROPIC_AUTH_TOKEN = connection.authToken
      const path = await expandHome(STATE_DIR + '/agent-settings/claude.json')
      if (Object.keys(env).length === 0) {
        // 没有要覆盖的就把旧文件删掉。不删的话，用户「清空 key」之后那份旧密钥
        // 还留在磁盘上 —— 功能上不会被读（argv 里不加 --settings），但把明文密钥
        // 留在那儿本身就是问题。
        try {
          await runCapture(['/bin/sh', '-c', 'rm -f ' + shellQuote(path)], 3000)
        } catch (error) { /* 删不掉也不该拦住这一轮 */ }
        return null
      }
      await writeSecretJsonFile(path, { env: env })
      return path
    }
    ...
    function persistStates() {
      ...
      armTimeout(() => {
        ...
        for (const [sessionId, state] of states) {
          ...
          // `defaults.model` 是 **claude 引擎**那份，不是 dsh 的 —— dsh 的模型由
          // dsh 自己那套设置管，插件不写它。所以「什么都没选过」的 dsh 会话就是
          // 出厂值那一份，可以直接跳过不存（见 stateOf 里的 defaultModelFor）。
    ...
    function launchSnapshot(state) {
      // 连接配置也算启动参数的一部分：改了它必须让**下一轮重建进程**，否则用户
      // 改完 baseUrl 会一直用旧的（broker 只在自己重启时才读新配置）。这正好是
      // 「下一轮生效」那条约定 —— 不主动打断正在跑的轮次。
      return state.permissionMode + '|' + state.effort + '|' + JSON.stringify(connectionOf('claude'))
    }
    ...
    async function ensureRun(sessionId, cwd, signal) {
      ...
      // 连接覆盖走 `--settings <文件>`：进程环境压不过用户 settings.json 里已有的
      // 键（实测），而这个压得过。只放要覆盖的几项，其余仍归用户的配置管。
      const settingsPath = await writeClaudeSettingsFile(connectionOf('claude'))
    ...
    /**
     * codex 的密钥环境变量名。取个不含 KEY / TOKEN / SECRET / PASSWORD 的名字是
     * 有意的：dsh 的 scrubbedParentEnv 会按那几个词剥**继承**来的环境变量，虽然
     * 显式传的 env 在它之后展开、不会被剥，但用一个中性名字等于多一层保险。
     */
    const CODEX_KEY_ENV = 'CCMODE_CODEX_CREDENTIAL'

    /**
     * config.toml 里 `model_provider` 指的那个 provider 名。
     *
     * 只做一次极简的正则提取：host 半边没有 node 导入（一切都走 shell），为一行
     * 配置引一个 TOML 解析器不划算 —— 这个键的形态就是 `model_provider = "名"`。
     */
    async function activeCodexProvider() {
      let text = ''
      try {
        text = String(await runCapture(['/bin/sh', '-c', 'cat "$HOME/.codex/config.toml" 2>/dev/null'], 3000) || '')
      } catch (error) { return '' }
    ...
    /**
     * codex 的连接覆盖，返回 `{ argv, env }`。
     *
     * **覆盖的是当前那个 provider，不新建**：用户 config.toml 里的 provider 可能
     * 带着必需的 `http_headers`（比如 opencode 的 `x-opencode-session`，缺了返回
     * 451），新建一个等于把这些丢掉、把本来能用的配置弄坏。所以只改 base_url 与
     * 密钥来源。
     *
     * 密钥不进 argv：`-c` 只写环境变量的**名字**（codex 的 `env_key`），真值走
     * 显式 env。这样 `ps` 看不到密钥 —— 而 claude 那边走 `--settings` 文件也是
     * 同一个理由。
     */
    async function codexConnectionArgs(connection) {
      ...
      if (baseUrl.length === 0 && apiKey.length === 0) return { argv: [], env: undefined }

      const asked = typeof connection.provider === 'string' ? connection.provider.trim() : ''
      const provider = asked.length > 0 ? asked : await activeCodexProvider()
      if (provider.length === 0) {
        console.error('cc-mode: 配了 Codex 的连接，但找不到要覆盖哪个 provider'
          + '（config.toml 里没有 model_provider，你也没在设置里指定）—— 这一项跳过')
        return { argv: [], env: undefined }
      }

      const argv = []
      if (baseUrl.length > 0) {
        argv.push('-c', 'model_providers.' + provider + '.base_url=' + JSON.stringify(baseUrl))
      }
      let env
      if (apiKey.length > 0) {
        argv.push('-c', 'model_providers.' + provider + '.env_key=' + JSON.stringify(CODEX_KEY_ENV))
        env = {}
        env[CODEX_KEY_ENV] = apiKey
      }
      console.log('cc-mode: codex 连接覆盖 → provider', provider,
        baseUrl.length > 0 ? '（base_url 已改）' : '', apiKey.length > 0 ? '（密钥走 ' + CODEX_KEY_ENV + '）' : '')
     ...
     */
    async function codexRound(cwd, argv, signal, transcript, onThreadId, env) {
      let executable
      try {
      ...
      try {
        proc = subprocess.spawn({
          ...
          stdio: { stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' },
          graceMs: 5000,
          // 连接覆盖时这里带一个环境变量（只含**密钥那一项**，见 codexConnectionArgs）。
          // 显式传的 env 会盖在 scrubbedParentEnv 之后，所以不会被敏感名过滤剥掉。
          ...(env === undefined ? {} : { env: env }),
    ...
    async function driveCodexTurn(agent, messages, turn, signal) {
      ...
      // 连接覆盖每轮算一次（读的是缓存，不额外碰磁盘）。
      const codexConn = await codexConnectionArgs(connectionOf('codex'))
      ...
      try {
        ...
        while (true) {
          ...
          const argv = codexArgv(state, threadId, prompt, imagePaths).concat(codexConn.argv)
    ...
    const disposers = [
      ...
      harness.handle('agents.set', (args) => agentsEdit({
        ...
        name: args.name,
        source: args.source,
        // 不传就是 undefined，applyAgentsEdit 据此保持原样（老的调用方只传三个字段）。
    ...
    // 引擎注册表也预热：connectionOf 是同步的，没热之前它会当成「什么都没覆盖」——
    // 那样第一轮就会用用户的默认配置跑，配了自定义 baseUrl 的人会看到「改完第一轮
    // 没生效」。放这里是因为它依赖的常量声明在本行之前。
    readAgentsFile()
      .catch((error) => console.error('cc-mode: could not read the engine registry:', errorText(error)))
