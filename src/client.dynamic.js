...
return {
  ...
  apply(ctx) {
    ...
    styles.insert(`
      ...
      .ccmode-set-conn { display:flex; flex-direction:column; gap:6px; min-width:0; }
      /* 连接字段底下那句说明长短不一（codex 的 provider 尤其长），所以这一行顶部
         对齐 —— 沿用面板里 flex-end 的话，说明长的那栏会把输入框顶到别处去。 */
      .ccmode-set-conn .ccmode-set-form { align-items:flex-start; }
      .ccmode-set-field-note { font-size:10.5px; line-height:1.4; color:var(--dsw-alias-label-tertiary, #888); }
      /* 密钥字段：输入框 + 那个「显示」开关并排，所以这里得自己撑开输入框的宽度。 */
      .ccmode-set-secret { display:flex; align-items:center; gap:6px; min-width:0; }
      .ccmode-set-secret > .ccmode-set-input { flex:1 1 auto; min-width:0; }
    `)
    ...
    // ---------- 设置面板：引擎 ----------
    //
    // 改的是显示名、来源标记和连接配置，不是执行方式：能被执行的引擎只有那三个
    // （host 的 ENGINE_IDS），所以这里只列它们，没有「新增引擎」这一说 —— 加一个
    // 跑不起来的 id 只会让人以为能用。保存后 refreshAgents() 会让引擎座、模型座
    // 分组标题、会话徽章一起改口。

    /**
     * 连接字段怎么画。
     *
     * **哪个引擎有哪些字段由 host 那份 connection 决定**（dsh 拿到的是 null，
     * 所以它一个字段都不画），这张表只管每个键长什么样 —— 字段清单不在前端再
     * 抄一份，host 那边加字段这里自动就有了。
     */
    const CONNECTION_FIELDS = {
      ...
      authToken: {
        ...
        secret: true,
        placeholder: '…',
        note: '留空 = 用该引擎自己的 token',
      },
      provider: {
        label: 'Provider',
        placeholder: 'config.toml 里的 provider 名',
        // 说清覆盖的边界：只改这两项，别处的配置原样留着 —— 否则没人敢填。
        // 末尾那半句是宿主真会跳过的情形（见 host 的 codexConnectionArgs）。
        note: '留空 = 跟随 config.toml 的 model_provider（那里也没有就跳过这两项覆盖）；'
          + '那个 provider 原有的 wire_api / http_headers 会保留，这里只覆盖 base_url 和 key',
      },
    }

    /** 每个引擎的「留空」到底落到哪个文件 —— 说清了人才能对上自己那份配置。 */
    const CONNECTION_DEFAULTS = {
      claude: '留空的字段不覆盖，仍然读 ~/.claude/settings.json 的 env（ANTHROPIC_BASE_URL / ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN）。',
      codex: '留空的字段不覆盖，仍然读 ~/.codex/config.toml 里那份 provider。',
    }
    ...
    function AgentRegistrySection() {
      ...
      const [error, setError] = React.useState('')
      const [busy, setBusy] = React.useState(false)
      // 每一行一个草稿：id -> { name, source, connection }。没编辑过的行不写草稿，
      // 直接显示 host 那份值，保存完就自然回到 host 的答案上。
      const [drafts, setDrafts] = React.useState({})
      // 密钥默认遮着。这是本机工具、key 是用户自己的，遮死没必要；但一开面板就
      // 把明文摊在屏幕上（截图、投屏、旁边有人）也没必要，所以给个逐字段的开关。
      ...
      function draftOf(entry) {
        return drafts[entry.id]
          || { name: entry.name, source: entry.source, connection: entry.connection }
      }

      function patch(entry, changes) {
        // 基准取整份草稿，而不是 drafts[entry.id]：只碰过一个字段的行，其余字段
        // 也得留在草稿里。少了这一层，「只改名字再保存」会把来源和连接当成
        // undefined 送出去 —— 前者被 host 拒掉，后者会被当成「不覆盖」。
        const base = draftOf(entry)
        setDrafts(Object.assign({}, drafts, { [entry.id]: Object.assign({}, base, changes) }))
      }

      /** 这一行的连接配置有没有被动过。逐字段比，不看对象引用。 */
      function connectionChanged(entry, draft) {
        const base = entry.connection
        // 没有连接配置的引擎（dsh）本来就不参与。
        if (base === null || base === undefined) return false
        const next = draft.connection || {}
        return Object.keys(base).some((key) => (next[key] || '') !== (base[key] || ''))
      }

      function save(entry) {
        const draft = draftOf(entry)
        const payload = { id: entry.id, name: draft.name, source: draft.source }
        // connection 是可选参数，不传就保持原样 —— 所以只有真的动过才带上：
        // 顺手也就不会把读回来的 key 原样在桥上跑一圈。
        if (connectionChanged(entry, draft)) payload.connection = draft.connection
        setBusy(true)
        host.call('agents.set', payload)
          .then((value) => {
            setState(value)
            setError('')
      ...
      /** 一个连接字段：标签 + 输入框（密钥另给一个显示开关）+ 那句「留空 = 什么」。 */
      function connectionField(entry, draft, key) {
        const spec = CONNECTION_FIELDS[key]
        const flag = entry.id + ':' + key
        const shown = revealed[flag] === true
        const input = h('input', {
          className: 'ccmode-set-input',
          // 密钥默认遮着；开关是逐字段的，看了一个不影响另一个。
          type: spec.secret === true && !shown ? 'password' : 'text',
          value: (draft.connection || {})[key] || '',
          disabled: busy,
          placeholder: spec.placeholder,
          onChange: (event) => patch(entry, {
            connection: Object.assign({}, draft.connection, { [key]: event.target.value }),
          }),
        })
        return h('label', { className: 'ccmode-set-field', key: key },
          h('span', { className: 'ccmode-set-field-label' }, spec.label),
          spec.secret === true
            ? h('div', { className: 'ccmode-set-secret' }, input,
                h('button', {
                  type: 'button',
                  className: 'ccmode-set-btn',
                  disabled: busy,
                  onClick: () => setRevealed(Object.assign({}, revealed, { [flag]: !shown })),
                }, shown ? '隐藏' : '显示'))
            : input,
          h('span', { className: 'ccmode-set-field-note' }, spec.note))
      }

      function agentRow(entry) {
        const draft = draftOf(entry)
        const dirty = draft.name !== entry.name || draft.source !== entry.source
          || connectionChanged(entry, draft)
        // connection 是 null 的引擎（只有 dsh）不画连接字段：它的模型和连接归
        // dsh 自己那套设置管，插件去写只会和它打架。
        const connectionFields = (entry.connection === null || entry.connection === undefined)
          ? null
          : h('div', { className: 'ccmode-set-conn' },
              h('div', { className: 'ccmode-set-hint' },
                CONNECTION_DEFAULTS[entry.id] || '留空的字段不覆盖，仍用该引擎自己的配置。'),
              h('div', { className: 'ccmode-set-form' },
                Object.keys(entry.connection)
                  .filter((key) => CONNECTION_FIELDS[key] !== undefined)
                  .map((key) => connectionField(entry, draft, key))))
            ...
            h('div', { className: 'ccmode-set-form' },
              h('label', { className: 'ccmode-set-field' },
                ...
                h('select', {
                  ...
                  value: draft.source,
                  disabled: busy,
                  onChange: (event) => patch(entry, { source: event.target.value }),
      ...
      return h('div', { className: 'ccmode-set' },
        h('div', { className: 'ccmode-set-block' },
          h('div', { className: 'ccmode-set-title' }, '引擎的显示名、来源与连接'),
          h('div', { className: 'ccmode-set-hint' },
            '显示名和来源只改界面上的字：三个引擎都由本机 CLI 驱动，来源眼下是注解'
            + '（本地 / 远程），以后接远程时才成为开关。连接配置只喂给对应的 CLI，'
            + '没填的引擎用自己那份配置文件 —— DSH 没有这一栏，它的连接由 dsh 自己那套设置管。'),
