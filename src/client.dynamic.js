...
return {
  ...
  apply(ctx) {
    ...
    function EngineChip(props) {
      ...
      const ENGINE_NAMES = { dsh: 'DSH', claude: 'Claude Code', codex: 'Codex' }
      return h(Select, {
        value: state.mode,
        onSelect: choose,
        align: 'left',
        ...
        label: 'DSH',
        title: locked
          ? ('这个会话已经是 ' + ENGINE_NAMES[state.mode] + ' 对话，不能改成别的引擎——几边的历史互不共享。要换请新建会话。')
          : (state.mode === 'dsh'
              ? 'DSH 原版循环。切换到 Claude Code / Codex 只在会话还没开始时可用。'
              : '本机 ' + ENGINE_NAMES[state.mode] + ' CLI 驱动这个会话；转录、工具卡片、审批都走 dsh 原生渲染。发出第一轮后就固定下来。'),
        options: [
          { id: 'dsh', name: 'DSH', detail: '原版 harness 循环' },
          { id: 'claude', name: 'Claude Code', detail: '本机 claude CLI 驱动，dsh 只负责渲染' },
          { id: 'codex', name: 'Codex', detail: '本机 codex CLI 驱动，dsh 只负责渲染' },
        ],
      })
    }
    ...
    function ClaudePostureSeat(props) {
      ...
      if (state.mode === 'codex') {
        ...
        return h(Select, {
          ...
          align: 'left',
          label: '沙箱',
          title: 'Codex 的沙箱档，替代 dsh 的访问档（下一轮 spawn 时生效）',
    ...
    function ClaudeModelSeat(props) {
      ...
      const models = state.mode === 'codex'
        ? allModels.filter((entry) => !/^claude-/.test(entry.id))
        : allModels
      ...
      const modelLabel = state.model.length > 0 && current
        ...
        : (state.mode === 'codex' ? '跟随 codex 配置' : routeLabel(state.route, models))
            ...
            : pane === 'model'
              ? h('div', { className: SEL.groups },
                  h('div', { className: SEL.group },
                    ...
                    // 那份清单本来就是这个引擎自己的模型。
                    // 不引用 EngineChip 里的 ENGINE_NAMES —— 那个常量在另一个
                    // 函数作用域里，这里拿不到。这个座位只服务 claude / codex。
                    h('div', { className: SEL.groupTitle }, state.mode === 'codex' ? 'Codex' : 'Claude Code'),
    ...
    function paintBadges() {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        const label = engine === 'codex' ? 'Codex 驱动的对话' : 'Claude Code 驱动的对话'
    ...
    function ModelCatalogSection() {
      ...
      // 必然失败的选项。
      const defaultOptions = (engine) => (engine === 'codex' ? [{ id: '', label: '跟随 codex 自己的配置' }] : [])
        .concat(all
      ...
      return h('div', { className: 'ccmode-set' },
        h('div', { className: 'ccmode-set-block' },
          ...
          defaultRow('Claude / DSH', '模型座里选过之后，这里会跟着变。',
            'claude', state.defaults.claude, defaultOptions('claude')),
          defaultRow('Codex', 'claude-* 在 Codex 上是必然 401，所以只列非 Claude 的模型。',
